...
class startMainFile():
    ...
    def shutdown_func(self):
        ...
        if self.startup == True:
            print("[Mainfile-shutdown func] Devices not started")
        else:
                ...
                try:
                    if 1 <= stereo_Distance <= 35:  # Meters
                        ...
                        if not self.first_measurement:
                            ...
                            self.distances = np.array([new_distance])
                        ...
                        try:
                            if len(self.distances) == self.max_measures:
                                ...
                                if abs(new_distance - moving_avgs[15]) <= 2.0:
                                    self.distances = np.append(self.distances, new_distance)
                                    self.replacements = 0
...
#                                        print("in try")
                                        ...
                                        poly_fit = np.poly1d(np.polyfit(x_values, self.distances[0:-1], 2, full=False))
#                                        print("1")
...
#                                        print(future_distance)
                                        ...
                                        difference = future_distance - self.distances[-1]
#                                        print("1")
                                        boost_scaling = 2.5*(self.distances[-1]/25)
#                                        print("2")
                                        if moving_avgs[15] - moving_avgs[5] < 1.5:
                                            future_distance = new_distance
                                        if abs(difference) > boost_scaling:
#                                            print("3")
                                            if difference > 0:
#                                                print("4")
                                                future_distance = self.distances[-1] + boost_scaling
#                                                print("5")
...
#                                                print("7")
                                        ...
                                        self.future_distance = round(future_distance,2)
                                        print("FUTURECALC  ",self.future_distance)
                                        self.got_future_dist = True
#
                                    except:
                                        print("issue getting prediction")
                                        pass
                                # ========================================================================= #


                                # DETECT RUNNiNG PLAYER:
#                                if abs(moving_avgs[15] - moving_avgs[0]) > 1.6 and not self.player_running.is_set():
